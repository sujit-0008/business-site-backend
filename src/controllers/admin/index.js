...
router.post("/products/:productId/approve", async (req, res) => {
  ...
  try {
    ...
    if (approved) {
      ...
      const subject = "Your Product Has Been Rejected";
      const message = `Dear ${supplier.ownerName} unfortunetly,\n\nYour product "${product.name}" (ID: ${productId}) has been Rejected.`;
      await sendEmail(supplier.email, subject, message).catch((err) => console.error("Email failed:", err.message));
    }
...
//kyc Approved suppliers to list on Admin dashboard!
